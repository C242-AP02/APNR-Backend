...
import { deleteFromFirestore, getTotalVehicle, getTotalVehicleDaily, getTotalVehicleMonthly, getTotalVehiclePerRegion, getUserPlateData, getVehicleById, saveToFirestore, setVehiclePublic } from "./firestore.js";
...
export async function handleGetPublicDetail(req, res) {
  try {
    const { plateDataId } = req.params;
    const result = await getVehicleById(plateDataId);
...
export async function handleSetPublicDetails(req, res) {
  try {
    const { uid } = req.cookies;
    const { plateDataId } = req.params
    ...
    await setVehiclePublic(plateDataId, true);

    return res.status(200).json({ message: "Vehicle details successfully made public" });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
}

export async function handleSetPrivateDetails(req, res) {
  try {
    const { uid } = req.cookies;
    const { plateDataId } = req.params
    ...
    await setVehiclePublic(plateDataId, false);

    return res.status(200).json({ message: "Vehicle details successfully made private" });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
}
